...
def write_obj_region(parno, data_dir, region_file_path, catalog, regfile_name, xoffset, yoffset, w, b_width, b_length):
    file = open(data_dir + region_file_path + f'Par{parno}{regfile_name}', 'a')
...
def create_regions(parno, args):
    ...
    spec_cat = glob.glob(args.data_dir + args.speccat_file_path + 'Par*spec*.fits')
    hdul = fits.open(spec_cat[0])
    cat=hdul[1].data

    # Direct image region files
    f = open(args.data_dir + args.region_file_path + f'Par{parno}regions_phot.reg','a')
    ...
    f115grism_R = glob.glob(args.data_dir + args.drizzled_images_path + '*f115w*gr150r_drz_sci.fits')
    if len(f115grism_R) != 0:
        write_obj_region(parno, args.data_dir, args.region_file_path, cat, 'F115r_grism.reg', 0.6548681566074263, 33.73739138173772, w = WCS(f115grism_R[0]),
                        b_width = 10.0, b_length = 93.54)
    
    f115grism_C = glob.glob(args.data_dir + args.drizzled_images_path + '*f115w*gr150c_drz_sci.fits') # Added KVN 19-Aug-2024
    if len(f115grism_C) != 0:
        write_obj_region(parno, args.data_dir, args.region_file_path, cat, 'F115c_grism.reg', 31.91107156101387, 1.3922939626209256, w = WCS(f115grism_C[0]),
                        b_width = 97.28751330105166, b_length = 10.0)

    f150grism_R = glob.glob(args.data_dir + args.drizzled_images_path + '*f150w*gr150r_drz_sci.fits') # Added KVN 19-Aug-2024
    if len(f150grism_R) != 0:
        write_obj_region(parno, args.data_dir, args.region_file_path, cat, 'F150r_grism.reg', 0.6548681566074263, 106.79254657227568, w = WCS(f150grism_R[0]),
                        b_width = 10.0, b_length = 93.54)
    
    f150grism_C = glob.glob(args.data_dir + args.drizzled_images_path + '*f150w*gr150c_drz_sci.fits') # Added KVN 19-Aug-2024
    if len(f150grism_C) != 0:
        write_obj_region(parno, args.data_dir, args.region_file_path, cat, 'F150c_grism.reg', 96.44444, 0.6548681566074263, w = WCS(f150grism_C[0]),
                        b_width = 93.54, b_length = 10.0)
    
    f200grism_R = glob.glob(args.data_dir + args.drizzled_images_path + '*f200w*gr150r_drz_sci.fits') # Added KVN 19-Aug-2024
    if len(f200grism_R) != 0:    
        write_obj_region(parno, args.data_dir, args.region_file_path, cat, 'F200r_grism.reg', 0.6548681566074263, 204.8370874255101, w = WCS(f200grism_R[0]),
                        b_width = 10.0, b_length = 131.78)        
    
    f200grism_C = glob.glob(args.data_dir + args.drizzled_images_path + '*f200w*gr150c_drz_sci.fits') # Added KVN 19-Aug-2024
    if len(f200grism_C) != 0:
        write_obj_region(parno, args.data_dir, args.region_file_path, cat, 'F200c_grism.reg', 200.9228, 0.6548681566074263, w = WCS(f200grism_C[0]),
